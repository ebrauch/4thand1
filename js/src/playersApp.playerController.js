...
function playerController($scope, $http) {
    ...
    $scope.addPlayer = function() {
        ...
        $http.get('/api/players/' + $scope.newPlayer.player)
            .then(function(serverResponse){
                serverResponse.data.forEach(function(data){
                    switch (data.dir) {
                        case 'LE':  $scope.newPlayer.leAtt++;
                                    $scope.newPlayer.leYds += data.yds;
                                    break;
                        case 'LT':  $scope.newPlayer.ltAtt++;
                                    $scope.newPlayer.ltYds += data.yds;
                                    break;
                        case 'LG':  $scope.newPlayer.lgAtt++;
                                    $scope.newPlayer.lgYds += data.yds;
                                    break;
                        case 'MD':  $scope.newPlayer.mdAtt++;
                                    $scope.newPlayer.mdYds += data.yds;
                                    break;
                        case 'RG':  $scope.newPlayer.rgAtt++;
                                    $scope.newPlayer.rgYds += data.yds;
                                    break;
                        case 'RT':  $scope.newPlayer.rtAtt++;
                                    $scope.newPlayer.rtYds += data.yds;
                                    break;
                        case 'RE':  $scope.newPlayer.reAtt++;
                                    $scope.newPlayer.reYds += data.yds;
                                    break;
                    }
                    switch (data.loc) {
                        case 'SL':  $scope.newPlayer.slTrg++;
                                    $scope.newPlayer.slYds += data.yds;
                                    break;
                        case 'SM':  $scope.newPlayer.smTrg++;
                                    $scope.newPlayer.smYds += data.yds;
                                    break;
                        case 'SR':  $scope.newPlayer.srTrg++;
                                    $scope.newPlayer.srYds += data.yds;
                                    break;
                        case 'DL':  $scope.newPlayer.dlTrg++;
                                    $scope.newPlayer.dlYds += data.yds;
                                    break;
                        case 'DM':  $scope.newPlayer.dmTrg++;
                                    $scope.newPlayer.dmYds += data.yds;
                                    break;
                        case 'DR':  $scope.newPlayer.drTrg++;
                                    $scope.newPlayer.drYds += data.yds;
                                    break;
                    }
                    if (data.psr == $scope.newPlayer.player) {
                        switch (data.loc) {
                            case 'SL':  $scope.newPlayer.slPaA++;
                                        $scope.newPlayer.slPaY += data.yds;
                                        break;
                            case 'SM':  $scope.newPlayer.smPaA++;
                                        $scope.newPlayer.smPaY += data.yds;
                                        break;
                            case 'SR':  $scope.newPlayer.srPaA++;
                                        $scope.newPlayer.srPaY += data.yds;
                                        break;
                            case 'DL':  $scope.newPlayer.dlPaA++;
                                        $scope.newPlayer.dlPaY += data.yds;
                                        break;
                            case 'DM':  $scope.newPlayer.dmPaA++;
                                        $scope.newPlayer.dmPaY += data.yds;
                                        break;
                            case 'DR':  $scope.newPlayer.drPaA++;
                                        $scope.newPlayer.drPaY += data.yds;
                                        break;
                        }
                    }
                })
        ...
        //$scope.newPlayerData.data.forEach(function(data){
        //    if (data.dir == 'RT') {
